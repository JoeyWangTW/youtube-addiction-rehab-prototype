...
function evaluateVideoRelevance(userGoal, videoTitle, apiKey, callback) {
    ...
    .catch(error => {
      ...
      callback({ evaluation_rating: 'not_sure', evaluation_context: 'Error occurred while evaluating.' });
    });
  }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.message === 'contentScriptReady') {
    ...
    // Now that the content script is ready, send the 'getVideoDetails' message
    sendGetVideoDetailsMessage(tabId);
  }
});
